...
@implementation WXPlusURLRewriteImpl
...
NSString *pattern=   @"^([A-Za-z0-9+/]{4})*([A-Za-z0-9+/]{4}|[A-Za-z0-9+/]{3}=|[A-Za-z0-9+/]{2}==)$";
    ...
    if(array.count>0){
        NSURL *strUrl = [NSURL URLWithString: [url stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet
...
{
    return[[[url scheme] lowercaseString] isEqualToString:@"local"];
}
